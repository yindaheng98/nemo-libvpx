...
int vpx_realloc_scaled_frame_buffer(YV12_BUFFER_CONFIG *ybf, int width,
                                    int crop_width, int height, int crop_height,
                                    int scale, int ss_x, int ss_y,
#if CONFIG_VP9_HIGHBITDEPTH
                                    int use_highbitdepth,
#endif
                                    int border, int byte_alignment,
  ...
  if (ybf) {
    ...
    ybf->y_crop_width = crop_width * scale;
    ybf->y_crop_height = crop_height * scale;
    ybf->y_width = aligned_width;
    ybf->y_height = aligned_height;
    ybf->y_stride = y_stride;

    ybf->uv_crop_width = (crop_width * scale + ss_x) >> ss_x;
    ybf->uv_crop_height = (crop_height * scale + ss_y) >> ss_y;
    ybf->uv_width = uv_width;
    ybf->uv_height = uv_height;
    ybf->uv_stride = uv_stride;
