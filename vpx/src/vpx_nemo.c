...
int read_cache_profile_dummy_bits(nemo_cache_profile_t *cache_profile) {
  ...
  if (fread(&cache_profile->num_dummy_bits, sizeof(int), 1,
            cache_profile->file) != 1) {
...
int RGB24_realloc_frame_buffer(RGB24_BUFFER_CONFIG *rbf, int width,
                               int height) {
  if (rbf) {
    ...
    if (frame_size > rbf->buffer_alloc_sz) {
      ...
      rbf->buffer_alloc =
          (uint8_t *)vpx_calloc(1, (size_t)frame_size * sizeof(uint8_t));
      if (!rbf->buffer_alloc) {
        return -1;
      }

      rbf->buffer_alloc_float =
          (float *)vpx_calloc(1, (size_t)frame_size * sizeof(float));
