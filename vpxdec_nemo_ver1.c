...
/* NEMO: New arguments */
static const arg_def_t datadirarg =
        ARG_DEF(NULL, "data-dir", 1, "Data directory");
static const arg_def_t contentarg =
        ARG_DEF(NULL, "content", 1, "Content");
static const arg_def_t dnnqualityarg =
        ARG_DEF(NULL, "dnn-quality", 1, "DNN quality");
static const arg_def_t inputresolutionarg =
        ARG_DEF(NULL, "input-resolution", 1, "Input video resolution");
static const arg_def_t referenceresolutionarg =
        ARG_DEF(NULL, "reference-resolution", 1, "Reference video resolution (for quality measurement)");
...
{   &help, &codecarg, &use_yv12, &use_i420, &flipuvarg, &rawvideo, &noblitarg, &progressarg, &limitarg, &skiparg,
    ...
    &datadirarg, &contentarg, &dnnqualityarg, &inputresolutionarg, &referenceresolutionarg,
    &outputwidtharg, &outputheightarg, &cachemodearg,
...
static const char *get_video_name(int resolution) {
    ...
    else die("Unsupported resolution: %d", resolution);
}

static const char *get_dnn_name(int resolution, nemo_dnn_quality dnn_quality) {
    if (resolution == 240) {
        if (dnn_quality == LOW) return "NEMO_S_B8_F9_S4_deconv";
        else if (dnn_quality == MEDIUM) return "NEMO_S_B8_F21_S4_deconv";
        else if (dnn_quality == HIGH) return "NEMO_S_B8_F32_S4_deconv";
    }
    else if (resolution == 360) {
        if (dnn_quality == LOW) return "NEMO_S_B4_F8_S3_deconv";
        else if (dnn_quality == MEDIUM) return "NEMO_S_B4_F18_S3_deconv";
        else if (dnn_quality == HIGH) return "NEMO_S_B4_F29_S3_deconv";
    }
    else if (resolution == 480) {
        if (dnn_quality == LOW) return "NEMO_S_B4_F4_S2_deconv";
        else if (dnn_quality == MEDIUM) return "NEMO_S_B4_F9_S2_deconv";
        else if (dnn_quality == HIGH) return "NEMO_S_B4_F18_S2_deconv";
    }
    else die("Unsupported resolution or dnn_quality: %d, %d", resolution, dnn_quality);
}

static void setup_directory(nemo_cfg_t *nemo_cfg, const char *data_dir, const char *content, int input_resolution, int reference_resolution,
                                nemo_dnn_quality quality, const char *postfix) {
    const char *input_video_name;
    const char *reference_video_name;
    const char *dnn_name;

        input_video_name = get_video_name(input_resolution);
        dnn_name = get_dnn_name(input_resolution, quality);

}

//TODO: support cache profile

static void set_dnn_quality(nemo_dnn_quality *dnn_quality, const char *dnn_quality_str)
{
    if (strcmp(dnn_quality_str, "low") == 0) {
        *dnn_quality = LOW;
    }
    else if (strcmp(dnn_quality_str, "medium") == 0) {
        *dnn_quality = MEDIUM;
    }
    else if (strcmp(dnn_quality_str, "high") == 0) {
        *dnn_quality = HIGH;
    }
    else {
        die("Unsupported dnn quality: %s", dnn_quality);
        exit(1);
    }
}
...
static int main_loop(int argc, const char **argv_)
{
  ...
  vpx_image_t *img_shifted = NULL;
    ...
    nemo_dnn_quality dnn_quality;
    const char *data_dir = NULL;
    const char *content = NULL;
    int input_resolution = 0, reference_resolution = 0;
    ...
    for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step)
    {
      ...
      ec_enabled = 1;
        ...
        /* NEMO: new arguments */
        else if (arg_match(&arg, &datadirarg, argi))
            data_dir = arg.val;
        else if (arg_match(&arg, &contentarg, argi))
            content = arg.val;
        else if (arg_match(&arg, &dnnqualityarg, argi))
            set_dnn_quality(&dnn_quality, arg.val);
        else if (arg_match(&arg, &inputresolutionarg, argi))
            input_resolution = atoi(arg.val);
        else if (arg_match(&arg, &referenceresolutionarg, argi))
            reference_resolution = atoi(arg.val);
    ...
    /* NEMO: setup */
    setup_directory(nemo_cfg, data_dir, content, input_resolution, reference_resolution, dnn_quality, postfix);
    sprintf(video_path, "%s/%s/video/%s", data_dir, content, get_video_name(input_resolution));
    sprintf(dnn_path, "%s/%s/checkpoint/%s/%s.dlc", data_dir, content, get_video_name(input_resolution), get_dnn_name(input_resolution, dnn_quality));
    scale = (int) floor (nemo_cfg->target_height / input_resolution);
  ...
  if (vp8_pp_cfg.post_proc_flag &&
    ...
    /* NEMO: Load a DNN */
     if(vpx_load_nemo_dnn(&decoder, scale, dnn_path)){
