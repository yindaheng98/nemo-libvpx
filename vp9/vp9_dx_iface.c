...
static vpx_codec_err_t load_nemo_cfg(vpx_codec_alg_priv_t *ctx,
                                     nemo_cfg_t *nemo_cfg) {
  ...
  if (ctx->nemo_cfg->save_metadata || ctx->nemo_cfg->save_quality ||
      ctx->nemo_cfg->save_latency) {
...
static vpx_codec_err_t load_nemo_dnn(vpx_codec_alg_priv_t *ctx, int scale,
                                     const char *dnn_file) {
...
static vpx_codec_err_t load_nemo_cache_profile(vpx_codec_alg_priv_t *ctx,
                                               int scale,
  ...
  if ((ctx->nemo_cfg->cache_profile->file = fopen(cache_profile_path, "rb")) ==
      NULL) {
#ifdef __ANDROID_API__
...
static vpx_codec_err_t init_decoder(vpx_codec_alg_priv_t *ctx) {
  ...
  if (ctx->nemo_cfg == NULL) ctx->nemo_cfg = init_nemo_cfg();
  ctx->nemo_cfg->decode_mode = DECODE_CACHE;
  ctx->nemo_cfg->bilinear_coeff = init_bilinear_coeff(64, 64, ctx->scale);
  ...
  /* NEMO: open a quality log file, initialize frames used for quality
   * measurements */
  if (ctx->nemo_cfg->save_quality) {
...
static int YV12_save_frame_buffer(YV12_BUFFER_CONFIG *frame,
                                  const char *save_dir, const char *file_name) {
  char file_path[PATH_MAX] = { 0 };
...
static int YV12_load_frame_buffer(YV12_BUFFER_CONFIG *frame,
                                  const char *save_dir, const char *file_name) {
  char file_path[PATH_MAX] = { 0 };
  FILE *serialize_file = NULL;
...
static void save_input_rgbframe(VP9_COMMON *cm) {
  ...
  vpx_realloc_frame_buffer(
      scaled_yuv_frame, width, height, cm->subsampling_x, cm->subsampling_y,
#if CONFIG_VP9_HIGHBITDEPTH
      cm->use_highbitdepth,
#endif
      VP9_DEC_BORDER_IN_PIXELS, cm->byte_alignment, NULL, NULL, NULL);
  I420Scale(yuv_frame->y_buffer, yuv_frame->y_stride, yuv_frame->u_buffer,
            yuv_frame->uv_stride, yuv_frame->v_buffer, yuv_frame->uv_stride,
            ...
            scaled_yuv_frame->y_crop_width, scaled_yuv_frame->y_crop_height, 3);

  // convert a yuv frame to a rgb frame
  RGB24_realloc_frame_buffer(scaled_rgb_frame, width, height);
  YV12_to_RGB24(scaled_rgb_frame, scaled_yuv_frame, cm->color_space,
                cm->color_range);
...
static void save_input_yuvframe(VP9_COMMON *cm) {
  ...
  if (cm->show_frame) {
    ...
    sprintf(file_name, "%04d_%d", cm->current_video_frame,
  ...
  // save a yuv frame
  YV12_save_frame_buffer(scaled_yuv_frame, cm->nemo_cfg->input_frame_dir,
                         file_name);
}
...
static void save_sr_rgbframe(VP9_COMMON *cm) {
  ...
  if (cm->show_frame) {
    ...
    sprintf(file_path, "%s/%04d_%d.raw", cm->nemo_cfg->sr_frame_dir,
            cm->current_video_frame, cm->current_super_frame);
  }
...
static void save_sr_yuv_frame(VP9_COMMON *cm) {
  ...
  // save a yuv frame
  YV12_save_frame_buffer(scaled_yuv_frame, cm->nemo_cfg->sr_frame_dir,
                         file_name);
}

static void save_rgbframe(VP9_COMMON *cm) {
  switch (cm->nemo_cfg->decode_mode) {
    case DECODE: save_input_rgbframe(cm); break;
    case DECODE_SR: save_sr_rgbframe(cm); break;
    case DECODE_CACHE: save_sr_rgbframe(cm); break;
  }
}
...
static void save_yuvframe(VP9_COMMON *cm) {
  if (cm->show_frame) {
    if (cm->nemo_cfg->filter_interval == 0 ||
      ...
      switch (cm->nemo_cfg->decode_mode) {
        case DECODE: save_input_yuvframe(cm); break;
        case DECODE_SR: save_sr_yuv_frame(cm); break;
        case DECODE_CACHE: save_sr_yuv_frame(cm); break;
      }
    }
  }
}
...
static void save_input_quality(VP9_COMMON *cm) {
  ...
  vpx_realloc_frame_buffer(
      upscaled_frame, width, height, cm->subsampling_x, cm->subsampling_y,
#if CONFIG_VP9_HIGHBITDEPTH
      cm->use_highbitdepth,
#endif
      VP9_DEC_BORDER_IN_PIXELS, cm->byte_alignment, NULL, NULL, NULL);
  I420Scale(frame->y_buffer, frame->y_stride, frame->u_buffer, frame->uv_stride,
            frame->v_buffer, frame->uv_stride, frame->y_crop_width,
            frame->y_crop_height, upscaled_frame->y_buffer,
            upscaled_frame->y_stride, upscaled_frame->u_buffer,
            upscaled_frame->uv_stride, upscaled_frame->v_buffer,
            upscaled_frame->uv_stride, upscaled_frame->y_crop_width,
            upscaled_frame->y_crop_height, 3);

  // load a reference frame
  YV12_BUFFER_CONFIG *reference_frame = cm->yv12_reference_frame;
  vpx_realloc_frame_buffer(
      reference_frame, width, height, cm->subsampling_x, cm->subsampling_y,
#if CONFIG_VP9_HIGHBITDEPTH
      cm->use_highbitdepth,
#endif
      VP9_DEC_BORDER_IN_PIXELS, cm->byte_alignment, NULL, NULL, NULL);
  YV12_load_frame_buffer(reference_frame,
                         cm->nemo_cfg->input_reference_frame_dir, file_name);
...
#ifdef __ANDROID_API__
  LOGI("output,%d frame: %.4fdB", cm->current_video_frame - 1,
       psnr_stats.psnr[0]);
#else
...
static void save_sr_quality(VP9_COMMON *cm) {
  ...
  vpx_realloc_frame_buffer(
      sr_upscaled_frame, width, height, cm->subsampling_x, cm->subsampling_y,
#if CONFIG_VP9_HIGHBITDEPTH
      cm->use_highbitdepth,
#endif
      VP9_DEC_BORDER_IN_PIXELS, cm->byte_alignment, NULL, NULL, NULL);
  I420Scale(sr_frame->y_buffer, sr_frame->y_stride, sr_frame->u_buffer,
            sr_frame->uv_stride, sr_frame->v_buffer, sr_frame->uv_stride,
  ...
  vpx_realloc_frame_buffer(
      sr_compare_frame, width, height, cm->subsampling_x, cm->subsampling_y,
#if CONFIG_VP9_HIGHBITDEPTH
      cm->use_highbitdepth,
#endif
      VP9_DEC_BORDER_IN_PIXELS, cm->byte_alignment, NULL, NULL, NULL);
  YV12_load_frame_buffer(sr_compare_frame, cm->nemo_cfg->sr_reference_frame_dir,
                         file_name);
...
static void save_quality(VP9_COMMON *cm) {
  switch (cm->nemo_cfg->decode_mode) {
    case DECODE: save_input_quality(cm); break;
    case DECODE_SR: save_sr_quality(cm); break;
    case DECODE_CACHE: save_sr_quality(cm); break;
  }
}

// TODO: add interp, sr
static void save_latency(VP9Decoder *pbi, int video_frame_index,
  ...
  for (i = 0; i < num_threads; ++i) {
    ...
    //        super_frame_index, mwd->latency.interp_intra_block,
    ...
    sprintf(log, "%d\t%d\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n",
            ...
            mwd->latency.decode_intra_block, mwd->latency.decode_inter_block,
            mwd->latency.decode_inter_residual, mwd->latency.interp_intra_block,
            mwd->latency.interp_inter_block,
            mwd->latency.interp_inter_residual);
  ...
  if (pbi->common.apply_dnn == 0) {
    ...
    sprintf(log, "%d\t%d\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n", video_frame_index,
            ...
            pbi->common.latency.sr_convert_yuv_to_rgb,
            pbi->common.latency.sr_execute_dnn,
...
static void save_metadata(VP9Decoder *pbi, int current_video_frame,
  ...
  for (i = 0; i < num_threads; ++i) {
    ...
    sprintf(log, "%d\t%d\t%d\t%d\t%d\t%d\n", current_video_frame,
            current_super_frame, mwd->metadata.num_blocks,
            mwd->metadata.num_intrablocks, mwd->metadata.num_interblocks,
  ...
  if (pbi->common.frame_type == KEY_FRAME || pbi->common.intra_only) {
    sprintf(log, "%d\t%d\t%d\t%d\t%d\tkey_frame\n", current_video_frame,
            current_super_frame, pbi->common.apply_dnn, pbi->common.frame_type,
            pbi->common.intra_only);
  } else {
    if (pbi->common.show_frame == 0) {
      sprintf(log,
              "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\talternative_"
              "reference_frame\n",
  ...
  if (frame_count > 0) {
    ...
    for (i = 0; i < frame_count; ++i) {
      ...
      if (cm->show_frame == 0)
        ...
        current_video_frame = cm->current_video_frame - 1;
    ...
    while (data_start < data_end) {
      ...
      if (cm->nemo_cfg->save_yuvframe) save_yuvframe(cm);
      if (cm->nemo_cfg->save_latency) {
        save_latency(ctx->pbi, cm->current_video_frame - 1,
                     cm->current_super_frame);
      }
      if (cm->nemo_cfg->save_metadata)
        save_metadata(ctx->pbi, cm->current_video_frame - 1,
                      cm->current_super_frame);
    }
  }
  /* NEMO: Save logs */
...
CODEC_INTERFACE(vpx_codec_vp9_dx) = {
  ...
  { load_nemo_cfg, load_nemo_dnn, load_nemo_cache_profile }
};
