...
static void dec_build_inter_predictors(
  ...
  if (is_scaled) {
    ...
    // NEMO: up-scale a motion vector
    scaled_mv = vp9_scale_nemo_mv(&mv_q4, mi_x + x, mi_y + y, sf);  
    xs = sf->x_step_q4;
    ys = sf->y_step_q4;
  } else {
    // Co-ordinate of containing block to pixel precision.
...
static void decode_block(TileWorkerData *twd, VP9Decoder *const pbi, int mi_row,
  ...
  if (!is_inter_block(mi)) {
    ...
    dec_build_inter_predictors_sb(pbi, xd, mi_row, mi_col, false);
    /* NEMO: transfer super-resoluted pixels */
    if (!cm->apply_dnn) {
      ...
      dec_build_inter_predictors_sb(pbi, xd, mi_row, mi_col, true);
    }
    // Reconstruction
    int eobtotal = 0;
    int plane;
